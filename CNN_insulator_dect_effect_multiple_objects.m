...
minSize = k;
sigma = 1.01;
imbuf{1} = imresize(imread('yangben/experiments/9.jpg'),'OutputSize',[400 600]);
imbuf{2} = imresize(imread('yangben/experiments/11.jpg'),'OutputSize',[400 600]);
imbuf{3} = imresize(imread('yangben/experiments/1.jpg'),'OutputSize',[400 600]);
imbuf{4} = imresize(imread('yangben/experiments/4.jpg'),'OutputSize',[400 600]);
...
%{
boxes{ch}=boxesproposal(imbuf{ch});
boxes{ch}=boxes{ch}(boxes{ch}(:,5)>0.16,:);
boxes{ch}(:,[3,4])=boxes{ch}(:,[4,3]);
boxes{ch}(:,[1,2])=boxes{ch}(:,[2,1]);
boxes{ch}(:,3)=boxes{ch}(:,1)+boxes{ch}(:,3);
boxes{ch}(:,4)=boxes{ch}(:,2)+boxes{ch}(:,4);
%}
