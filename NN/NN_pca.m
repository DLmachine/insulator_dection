...
%% load training data
load Common/samples/train_images
load Common/samples/train_labels
for  i=1:size(labels,1)
    qq(:,:,i)=double(rgb2gray(images(:,:,:,i)))/256;
end
qq=reshape(qq,[64*64, size(labels,1)]);
%qq=loadMNISTImages('../common/train-images-idx3-ubyte');
x = sparse(qq);
[n m] = size(x);
...
if isOctave(); 
    ...
    rand('state',0);%rng('default'); % get numbers to match (most) of the figures  // ×¢Òâ£¬¿ÉÒÔÆÁ±Î
